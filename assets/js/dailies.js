class Dailies {
  ...
  static init() {
    ...
    if (localStorage.getItem('rdo.lastDailiesDate') !== currentDate) {
      for (const setting in localStorage) {
        if (setting.startsWith('rdo.dailies.')) localStorage.removeItem(setting);
      }
      localStorage.setItem('rdo.lastDailiesDate', currentDate);
    }

    return Promise.all([Loader.promises['dailies'].consumeJson(), Loader.promises['possible_dailies'].consumeJson()])
  ...
  static switchCategory(offset) {
    ...
    Array.from(difficultySelectors).forEach((selector) => {
      selector.classList.toggle('hidden', !selector.classList.contains(`${activeRole}-dailies-difficulty-selection`));
    });
  }
  ...
  static switchDifficulty() {
    ...
    dailiesGroup.forEach((daily) => {
      daily.classList.toggle('hidden', daily.getAttribute('data-type') !== `${activeRole}-${selectedDifficulty}`);
    });
  }
...
class SynchronizeDailies {
  ...
  sync() {
    if (!!DailyChallenges[this.challengeKey]) return;
    if (this.categoryType !== 'general' && this.categoryType !== DailyChallenges[`${this.role}_difficulty`]) return;
