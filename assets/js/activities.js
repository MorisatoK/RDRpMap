class Activities {
  static init() {
    this.locations = [];
    this.quickParams = [];
    this.context = document.querySelector('.menu-hidden[data-type=activities]');

    return Loader.promises['activities'].consumeJson(data => {
      data.forEach(item => {
        this.locations.push(new Activities(item));
        this.quickParams.push(item.key);
      });
      console.info('%c[Activities] Loaded!', 'color: #bada55; background: #242424');
  ...
  constructor(preliminary) {
    ...
    Object.assign(this.element.dataset, { help: 'item', type: this.key, tippyContent: Language.get(`map.activities.${this.key}.name`) });
    this.element.innerHTML = `
      ...
      <span class="collectible-text ${!this.onMap ? 'disabled' : ''}">
        <p class="collectible" data-text="map.activities.${this.key}.name"></p>
      </span>
    `;
    this.element.addEventListener('click', () => this.onMap = !this.onMap);
    Language.translateDom(this.element);

    Activities.context.appendChild(this.element);
  ...
  onLanguageChanged() {
    this.markers = [];
    this.locations.forEach(item => this.markers.push(new Marker(item.text, item.x, item.y, 'activities', this.key)));
  ...
  set onMap(state) {
    ...
    MapBase.updateTippy('activities');
  }
  get onMap() {
  ...
  static onLanguageChanged() {
    Activities.locations.forEach(infra => infra.onLanguageChanged());
    Menu.reorderMenu(this.context);
  }

  static onSettingsChanged() {
    Activities.locations.forEach(infra => infra.reinitMarker());
  }
}
